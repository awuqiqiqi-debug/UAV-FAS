...
html += '.chart{background:#fff;border:1px solid #e8e8e8;border-radius:6px;padding:6px;margin:8px 0;width:100%}\n'
html += '.grid2{display:grid;grid-template-columns:1fr 1fr;gap:12px;width:100%}\n'
html += '.grid3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;width:100%}\n'
html += '.grid2 .chart{min-height:250px}\n'
html += '@media(max-width:900px){.grid2{grid-template-columns:1fr}.grid3{grid-template-columns:1fr}}\n'
html += '.tbl-wrap{overflow-x:auto;margin-top:12px}\n'
html += '.tbl{width:100%;border-collapse:collapse;font-size:12px;min-width:800px}\n'
html += '.tbl th,.tbl td{padding:10px 14px;border:1px solid #e0e0e0;text-align:center;white-space:nowrap}\n'
...
html += '  <div class="grid3" id="traj_container"></div>\n'
...
# 图7 + 表1 合并到同一个section
# 计算热力图高度: 每行50px + 边距120px
heatmap_height = max(400, len(selected_eps) * 50 + 120)
html += '<div class="section">\n'
...
html += '  <div class="chart" id="allocation_chart" style="height:' + str(heatmap_height) + 'px"></div>\n'
html += '  <div class="section-caption">Fig.7 RIS unit allocation heatmap (green=User1, blue=User2, red=jam).</div>\n'
html += '  <div style="margin-top:20px;padding-top:16px;border-top:1px solid #e8e8e8">\n'
html += '    <div class="section-title" style="margin-bottom:8px">表1 &nbsp; 关键指标随训练轮次变化</div>\n'
html += '    <div class="tbl-wrap" style="max-height:300px;overflow-y:auto">\n'
html += '    <table class="tbl">\n'
html += '      <tr><th style="text-align:left;position:sticky;top:0;background:#302b63;z-index:1">指标</th>' + ep_headers + '</tr>\n'
...
html += 'var layout_base={font:{family:"Helvetica Neue,Arial,sans-serif",size:11,color:"#333"},paper_bgcolor:"#fff",plot_bgcolor:"#fafbff",margin:{t:40,b:45,l:55,r:20},height:300,autosize:true,legend:{bgcolor:"rgba(255,255,255,0.95)",bordercolor:"#ddd",borderwidth:1,font:{size:10}}};\n'
html += 'var plotConfig={responsive:true,displayModeBar:true,modeBarButtonsToRemove:["lasso2d","select2d"]};\n'
...
html += '    {x:[traj[ep].x[0]],y:[traj[ep].y[0]],mode:"markers",name:"起点",marker:{size:8,color:C.green,symbol:"triangle-up"}},\n'
html += '    {x:[traj[ep].x[traj[ep].x.length-1]],y:[traj[ep].y[traj[ep].y.length-1]],mode:"markers",name:"终点",marker:{size:8,color:C.red,symbol:"x",line:{width:2}}}\n'
html += '  ].concat(entityMarkers),Object.assign({},layout_base,{\n'
html += '    height:300,title:{text:"Episode "+ep,font:{size:10,color:"#333"}},\n'
html += '    xaxis:{title:{text:"X (m)",font:{size:9}},range:[-60,60],dtick:20,tickfont:{size:7},gridcolor:"#eee"},\n'
html += '    yaxis:{title:{text:"Y (m)",font:{size:9}},range:[-60,60],dtick:20,tickfont:{size:7},gridcolor:"#eee",scaleanchor:"x",scaleratio:1},\n'
html += '    shapes:[{type:"rect",x0:-50,x1:50,y0:-50,y1:50,line:{color:"#bbb",width:1,dash:"dot"},fillcolor:"rgba(0,0,0,0)"}],\n'
html += '    legend:{x:0.01,y:0.99,xanchor:"left",yanchor:"top",font:{size:7}}\n'
html += '  }),plotConfig);\n'
html += '});\n'
...
html += '],Object.assign({},layout_base,{\n'
html += '  height:350,\n'
html += '  xaxis:{title:{text:"训练轮次 (Episode)",font:{size:12}},tickfont:{size:10},gridcolor:"#eee",dtick:50},\n'
...
html += '],Object.assign({},layout_base,{\n'
html += '  height:' + str(heatmap_height) + ',\n'
...
html += '  margin:{t:30,b:50,l:80,r:80}\n'
html += '}),plotConfig);\n'
