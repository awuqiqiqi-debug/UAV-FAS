...
class DataManager(object):
    ...
    """
    def __init__(self, store_list = ['beamforming_matrix', 'reflecting_coefficient', 'UAV_state', 'user_capacity'],file_path = './data', store_path = './data/storage', project_name = None, existing_path = None):
        # 1 init location data
        self.store_list = store_list
        self.init_data_file = file_path + '/init_location.xlsx'
        
        # 如果指定了 existing_path，直接使用已有目录
        if existing_path is not None and os.path.isdir(existing_path):
            self.store_path = existing_path
    ...
    def read_init_location(self, entity_type = 'user', index = 0):
        if entity_type == 'user' or 'attacker' or 'RIS' or 'RIS_norm_vec' or 'UAV':
            ...
            pd.read_excel(self.init_data_file, sheet_name=entity_type)['z'][index]], dtype=float)
        else:
            return None
