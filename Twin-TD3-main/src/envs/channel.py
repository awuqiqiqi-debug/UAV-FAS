...
import math
import cmath
from src.envs.math_tool import *
...
class mmWave_channel(object):
    ...
    def get_estimated_channel_matrix(self):
        ...
        channel_matrix = np.asmatrix(np.ones(shape=(N_r,N_t),dtype=complex), dtype=complex)
    ...
    def generate_array_response(self, transceiver, theta, fai):
        ...
        if ant_type == 'UPA':
            ...
            col_num = ant_num // row_num
            Planar_response = np.asmatrix(np.ones(shape=(ant_num, 1)), dtype=complex)
            for i in range(row_num):
                for j in range(col_num):
                    Planar_response[j+i*col_num,0] = cmath.exp(1j *\
                    (math.sin(theta) * math.cos(fai)*i*math.pi + math.sin(theta)*math.sin(fai)*j*math.pi)\
                    )

            return Planar_response
        elif ant_type == 'ULA':
            Linear_response = np.asmatrix(np.ones(shape=(ant_num,1)), dtype=complex)
