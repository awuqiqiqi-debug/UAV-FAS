...
class minimal_IRS_system():
    def __init__(self, BS_M = 8, IRS_N_x = 4, IRS_N_y =2, K = 8, statistic = False):
        ...
        # transmit signal
        self.X_transmit = np.asmatrix(np.ones((K, 1)))       # BS transmit signal, unit variance, shape: K X 1

        # active beamforming at BS
        self.G_beamforming = np.asmatrix(np.ones((self.M, K),dtype=complex))    # BS beamforming matrix, shape: M X K
        self.BS_max_power = self.calculate_total_transmit_power()
        self.reset_G = np.asmatrix(np.ones((self.M, K),dtype=complex))

        # channal gain from BS to IRS
        #self.H_BS_to_IRS = np.asmatrix(np.ones((self.N, self.M)))      # channel gain BS to IRS, shape: N X M
        self.H_BS_to_IRS = self.calculate_channel_BS_to_IRS(self.IRS)

        # channal gain from IRS to users
        # self.H_IRS_to_user = np.asmatrix(np.ones((K, self.N),dtype=complex))    # channel gain IRS to user, shape: K X N
        self.H_IRS_to_user = self.calculate_channel_IRS_to_user(self.IRS)

        # IRS phase shifter parameter
        self.Fai = np.asmatrix(np.identity(self.N), dtype=complex)               # IRS phase shift 
        self.reset_Fai = np.asmatrix(np.identity(self.N), dtype=complex)
        # data rate matrex container
        self.data_rate = np.asmatrix(np.ones((K,1)))         # channal capacities for all K users
    ...
    def calculate_received_signal_amplitude(self):
        ...
        noise_matrix = np.asmatrix(noise_array)
    ...
    def calculate_channel_BS_to_IRS(self, IRS): # IRS elements spacing half wavelength
        ...
        temp_horizontal_matrix = np.asmatrix(temp_horizontal_matrix)
        temp_vertical_matrix = np.asmatrix(temp_vertical_matrix)
