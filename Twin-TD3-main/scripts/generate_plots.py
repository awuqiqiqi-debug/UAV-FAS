"""Generate all plots for any experiment directory.
Usage: python scripts/generate_plots.py --path data/storage/scratch/td3_mfris_jam_v5
"""
import argparse
import matplotlib
matplotlib.use('Agg')
...
parser.add_argument('--path', type=str, required=True, help='Path to experiment data directory')
parser.add_argument('--ris-pos', type=float, nargs=2, default=[12.0, 35.0], help='RIS position [x, y]')
parser.add_argument('--uav-start', type=float, nargs=2, default=[-10.0, 10.0], help='UAV start position [x, y]')
parser.add_argument('--max-ep', type=int, default=2001, help='Max episode number to scan')
args = parser.parse_args()

path = args.path
plot_dir = os.path.join(path, 'plot')
os.makedirs(os.path.join(plot_dir, 'RIS'), exist_ok=True)
ris_pos = np.array(args.ris_pos)
uav_start = np.array(args.uav_start)
...
for ep in range(args.max_ep):
    ...
    uc = np.array(result['user_capacity'][0,0])
    sc = np.array(result['secure_capacity'][0,0])
    ac = np.array(result['attaker_capacity'][0,0])
    um = np.array(result['UAV_movement'][0,0])
...
colors = plt.cm.plasma(np.linspace(0.1, 0.9, max(len(ep_traj), 1)))
for (ep, traj), color in zip(sorted(ep_traj.items()), colors):
    a = 0.3 + 0.7 * (ep / max(args.max_ep, 1))
    lw = 0.8 + 1.5 * (ep / max(args.max_ep, 1))
...
ax.set_title('UAV Trajectory', color='#60a5fa', fontsize=TITLE_SIZE, pad=15)
...
# 10. RIS phase plots
f_last = os.path.join(path, f'simulation_result_ep_{len(all_rewards)-1}.mat')
if os.path.exists(f_last):
    data = scipy.io.loadmat(f_last)
    ep_key = [k for k in data.keys() if k.startswith('result_')][-1]
    result = data[ep_key]
    ...
    for elem in range(n_elem):
        ...
        ax.set_title(f'RIS Element #{elem} Phase & Magnitude', fontsize=TITLE_SIZE, pad=12, color='#222222', fontweight='bold')
        style_ax(ax)
