...
# 直接指定训练目录
DATA_DIR = 'data/storage/uav_bs_fas/scratch/td3_see_50'

if not os.path.exists(DATA_DIR):
    print(f"训练目录不存在: {DATA_DIR}")
    exit(1)
...
print(f"找到训练目录: {DATA_DIR}")
print(f"最大episode数: {MAX_EPISODES}")
print(f"总文件数: {NUM_FILES}")

# 检查训练是否完成（目标1000轮）
TARGET_EPISODES = 1000
if MAX_EPISODES < TARGET_EPISODES:
    print(f"\n警告: 训练尚未完成！当前进度: {MAX_EPISODES}/{TARGET_EPISODES}")
    print("请等待训练完成后再运行此脚本。")
    # 仍然继续生成部分报告
else:
    print(f"\n训练已完成！共 {MAX_EPISODES} 轮")
...
for f in ep_files:
    ...
    if key in data:
        ...
        episode_uav_positions.append(uav[-1, :2])
...
if episode_capacities.ndim == 2:
    print(f"  平均容量: {np.mean(episode_capacities, axis=0)}")
else:
    print(f"  平均容量: {np.mean(episode_capacities):.4f}")

# 加载轨迹数据（选择有完整轨迹的episode）
all_uav轨迹 = []
# 选择5个代表性episode: 早期、中期、后期
target_eps = [50, 200, 400, 600, 900]
for ep in target_eps:
    f = f'simulation_result_ep_{ep}.mat'
    if f in os.listdir(DATA_DIR):
        data = loadmat(os.path.join(DATA_DIR, f))
        key = f'result_{ep}'
        if key in data:
            result = data[key]
            ...
            if uav.shape[0] >= 10:
                ...
                print(f"  加载轨迹: ep {ep}, {uav.shape[0]} 步")
...
border_x = [-40, 40, 40, -40, -40]
border_y = [0, 0, 60, 60, 0]
...
uav_start = system.UAV_BS_FAS.coordinate
ax1.scatter(uav_start[0], uav_start[1], c='blue', s=200, marker='s', edgecolors='black',
            linewidth=2, zorder=5, label=f'UAV起点 ({uav_start[0]:.0f}, {uav_start[1]:.0f})')
...
ax = axes[0, 1]
if episode_capacities.ndim == 2 and episode_capacities.shape[1] >= 2:
    ...
    ax.plot(episode_capacities, 'g-', linewidth=1.5, label='用户容量')
ax.set_xlabel('训练轮次', fontsize=11)
...
ax = axes[1, 0]
if episode_secure_caps.ndim == 2 and episode_secure_caps.shape[1] >= 2:
    ...
    ax.plot(episode_secure_caps, 'g-', linewidth=1.5, label='安全容量')
ax.set_xlabel('训练轮次', fontsize=11)
...
ax = axes[1, 1]
if episode_capacities.ndim == 2 and episode_secure_caps.ndim == 2 and episode_capacities.shape[1] >= 2:
    categories = ['用户1\n总容量', '用户1\n安全容量', '用户2\n总容量', '用户2\n安全容量']
    ...
    colors_bar = ['#2196F3', '#4CAF50', '#FF9800', '#8BC34A']
else:
    categories = ['平均总容量', '平均安全容量']
    values = [np.mean(episode_capacities), np.mean(episode_secure_caps)]
...
ax = axes[1, 0]
from src.envs.uav_comm_env import MiniSystem as MS
...
训练参数:
  ...
  - 训练轮次: {episodes} episodes x 100 steps
...
# 计算统计指标
final_10_reward = np.mean(episode_rewards[-10:]) if len(episode_rewards) >= 10 else np.mean(episode_rewards)
final_10_capacity = np.mean(episode_capacities[-10:], axis=0) if episode_capacities.ndim == 2 and len(episode_capacities) >= 10 else np.mean(episode_capacities)
final_10_secure = np.mean(episode_secure_caps[-10:], axis=0) if episode_secure_caps.ndim == 2 and len(episode_secure_caps) >= 10 else np.mean(episode_secure_caps)

# 容量占比
if episode_capacities.ndim == 2:
    cap_ratio = np.mean(episode_secure_caps[:, 0])/(np.mean(episode_capacities[:, 0])+1e-10)*100
    cap1_mean = np.mean(episode_capacities[:, 0])
    sec1_mean = np.mean(episode_secure_caps[:, 0])
else:
    cap_ratio = np.mean(episode_secure_caps)/(np.mean(episode_capacities)+1e-10)*100
    cap1_mean = np.mean(episode_capacities)
    sec1_mean = np.mean(episode_secure_caps)
...
<body>
    <div class="container">
        ...
        <div class="grid">
            ...
            <div class="stat-card">
                <div class="stat-value">{final_10_reward:.3f}</div>
                <div class="stat-label">最终10轮平均奖励</div>
            </div>
            <div class="stat-card">
                <div class="stat-value" style="color:#34d399;">{cap1_mean:.4f}</div>
                <div class="stat-label">用户1平均容量</div>
            </div>
            <div class="stat-card">
                <div class="stat-value" style="color:#fbbf24;">{sec1_mean:.4f}</div>
...
安全容量: C_s = max(0, R_user - R_attacker)
        ...
        <div class="card">
            ...
            <div class="chart-section">
                ...
                <div class="chart-desc">
                    展示TD3强化学习算法在{len(episodes)}轮训练过程中的性能变化，包括奖励曲线、用户容量、安全容量和容量对比。
                </div>
            ...
            <div class="grid">
                <div>
                    ...
                    <div class="metric">
                        ...
                        <span class="metric-value">{final_10_reward:.4f}</span>
                    </div>
                </div>
                <div>
                    <h3>容量指标统计</h3>
                    <div class="metric">
                        ...
                        <span class="metric-value">{cap1_mean:.6f} bps/Hz</span>
                    </div>
                    <div class="metric">
                        <span class="metric-label">用户1平均安全容量</span>
                        <span class="metric-value">{sec1_mean:.6f} bps/Hz</span>
                    </div>
                    <div class="metric">
                        <span class="metric-label">安全容量占比</span>
                        <span class="metric-value">{cap_ratio:.1f}%</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="grid">
                <div>
                    ...
                    <table>
                        ...
                        <tr><td>反射单元数</td><td>64 (8x8 UPA)</td></tr>
                        <tr><td>单元间距</td><td>0.5lambda ~ 5.36mm</td></tr>
                        <tr><td>法线方向</td><td>[0, 1, 0] (指向y轴)</td></tr>
                        <tr><td>反射矩阵</td><td>64x64对角矩阵</td></tr>
                    </table>
                </div>
                <div>
                    <h3>波束成形配置</h3>
