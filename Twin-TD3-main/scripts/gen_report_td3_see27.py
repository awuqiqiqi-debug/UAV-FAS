"""生成td3_see_27的学术论文级HTML训练报告"""
import json, scipy.io as sio, numpy as np, os, csv

mat_dir = 'data/storage/uav_bs_fas/scratch/td3_see_27'
...
<head>
<meta charset="UTF-8">
<title>FAS-UAV保密通信TD3训练报告 (td3_see_27)</title>
...
<div class="title-block">
  ...
  <div class="subtitle">FAS-Assisted UAV Secure Communication System &mdash; Twin Delayed TD3 Training Report (td3_see_27)</div>
  <div class="meta">奖励函数: 改进版v3(RIS干扰多样性+窃听者惩罚) &nbsp;|&nbsp; 训练轮次: 350 &nbsp;|&nbsp; 每轮步数: 100</div>
</div>
...
<div class="footer">FAS-UAV Secure Communication &mdash; TD3 Reinforcement Learning Training Report (td3_see_27)</div>
...
Plotly.newPlot('c_reward', [
  ...
  height:350,
  xaxis:{{title:{{text:'训练轮次 (Episode)',font:{{size:13}}}},tickfont:{{size:11}},gridcolor:'#eee',dtick:50}},
  yaxis:{{title:{{text:'奖励 (Reward)',font:{{size:13}}}},tickfont:{{size:11}},gridcolor:'#eee'}}
}}));
...
risEps.forEach(function(ep){{
  ...
  if(risData[ep].fas_port){{traces.push({{x:x,y:risData[ep].fas_port,name:'FAS端口号',yaxis:'y2',line:{{color:C.orange,width:1.5}}}});}}
  Plotly.newPlot(div.id,traces,Object.assign({{}},layout_base,{{
