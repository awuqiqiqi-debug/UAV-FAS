"""生成td3_see_33的学术论文级HTML训练报告"""
import json, scipy.io as sio, numpy as np, csv

mat_dir = 'data/storage/uav_bs_fas/scratch/td3_see_33'
...
<head>
<meta charset="UTF-8">
<title>FAS-UAV保密通信TD3训练报告 (td3_see_33)</title>
...
<div class="title-block">
  ...
  <div class="subtitle">FAS-Assisted UAV Secure Communication System &mdash; Twin Delayed TD3 Training Report (td3_see_33)</div>
  <div class="meta">奖励函数: 合理设计版(保密速率+距离引导+FAS增益) &nbsp;|&nbsp; 干扰功率: 100mW &nbsp;|&nbsp; 训练轮次: 350</div>
</div>
<div class="summary">
...
<div class="figure">
  ...
  <div class="fig-caption">Fig.1 &nbsp; Training reward convergence curve.</div>
</div>
<div class="figure">
  <div class="fig-label">图2 &nbsp; 不同训练阶段无人机飞行轨迹</div>
  <div class="grid2" id="traj_container"></div>
  <div class="fig-caption">Fig.2 &nbsp; UAV trajectories at different training stages.</div>
</div>
<div class="figure">
  <div class="fig-label">图3 &nbsp; 用户与窃听者信道容量对比</div>
  <div class="grid2" id="cap_container"></div>
  <div class="fig-caption">Fig.3 &nbsp; 信道容量对比。窃听者容量(虚线) vs 用户容量(实线)。</div>
</div>
<div class="figure">
...
<div class="figure">
  ...
  <div class="fig-caption">Fig.5 &nbsp; RIS phase and FAS active port index.</div>
</div>
<div class="tbl-section">
  <table>
    <caption>表1 &nbsp; 训练关键指标对比</caption>
    <tr><th>指标</th><th>Episode 0</th><th>Episode 100</th><th>Episode 200</th><th>Episode 349</th></tr>
    ''' + rows_html + '''
  </table>
</div>
<div class="footer">FAS-UAV Secure Communication &mdash; TD3 Reinforcement Learning Training Report (td3_see_33)</div>
<script>
...
], Object.assign({}, layout_base, {height:350, xaxis:{title:{text:'训练轮次',font:{size:13}},dtick:50}, yaxis:{title:{text:'奖励',font:{size:13}}}}));

var trajEps = [0, 100, 200, 349];
...
var entityMarkers = [
  ...
  {x:[0],y:[50],name:'RIS',mode:'markers',marker:{size:11,color:'#fff',symbol:'square',line:{color:C.magenta,width:2}}}
];
trajEps.forEach(function(ep){if(!traj[ep])return;var div=document.createElement('div');div.className='chart';div.id='traj_'+ep;trajContainer.appendChild(div);var traces=[{x:traj[ep].x,y:traj[ep].y,mode:'lines',name:'UAV轨迹',line:{color:C.cyan,width:2}},{x:[traj[ep].x[0]],y:[traj[ep].y[0]],mode:'markers',name:'起点',marker:{size:10,color:C.green,symbol:'triangle-up'}},{x:[traj[ep].x[traj[ep].x.length-1]],y:[traj[ep].y[traj[ep].y.length-1]],mode:'markers',name:'终点',marker:{size:10,color:C.red,symbol:'x',line:{width:2}}}].concat(entityMarkers);Plotly.newPlot(div.id,traces,Object.assign({},layout_base,{height:340,title:{text:'Ep '+ep},xaxis:{range:[-80,80],dtick:20},yaxis:{range:[-60,60],dtick:20}}));});

var capEps=[0,100,200,349];var capContainer=document.getElementById('cap_container');
capEps.forEach(function(ep){if(!cap[ep])return;var div=document.createElement('div');div.className='chart';div.id='cap_'+ep;capContainer.appendChild(div);var x=[];for(var i=0;i<100;i++)x.push(i);Plotly.newPlot(div.id,[{x:x,y:cap[ep].u0,name:'用户0',line:{color:C.cyan,width:1.5}},{x:x,y:cap[ep].u1,name:'用户1',line:{color:C.green,width:1.5}},{x:x,y:cap[ep].a0,name:'窃听0',line:{color:C.red,width:1.5,dash:'dash'}},{x:x,y:cap[ep].a1,name:'窃听1',line:{color:C.orange,width:1.5,dash:'dash'}}],Object.assign({},layout_base,{title:{text:'Ep '+ep},yaxis:{title:{text:'bits/s/Hz'}}}));});

var secEps=[100,200,349];var secContainer=document.getElementById('sec_container');
secEps.forEach(function(ep){if(!sec[ep])return;var div=document.createElement('div');div.className='chart';div.id='sec_'+ep;secContainer.appendChild(div);var x=[];for(var i=0;i<100;i++)x.push(i);Plotly.newPlot(div.id,[{x:x,y:sec[ep].u0,name:'用户0',fill:'tozeroy',line:{color:C.cyan,width:1.5},fillcolor:'rgba(0,117,192,0.15)'},{x:x,y:sec[ep].u1,name:'用户1',fill:'tozeroy',line:{color:C.green,width:1.5},fillcolor:'rgba(0,117,0,0.1)'}],Object.assign({},layout_base,{title:{text:'Ep '+ep},yaxis:{title:{text:'bits/s/Hz'}}}));});

var risEps=Object.keys(risData).map(Number);var risContainer=document.getElementById('ris_container');
risEps.forEach(function(ep){if(!risData[ep])return;var div=document.createElement('div');div.className='chart';div.id='ris_'+ep;risContainer.appendChild(div);var traces=[];var x=[];for(var i=0;i<(risData[ep].ris_signal||[]).length;i++)x.push(i);if(risData[ep].ris_signal){traces.push({x:x,y:risData[ep].ris_signal,name:'RIS信号相位',line:{color:C.cyan,width:1.5}});}if(risData[ep].ris_jam){traces.push({x:x,y:risData[ep].ris_jam,name:'RIS干扰相位',line:{color:C.red,width:1.5,dash:'dash'}});}Plotly.newPlot(div.id,traces,Object.assign({},layout_base,{title:{text:'Ep '+ep},height:380,yaxis:{range:[-1.2,1.2]}}));});
</script>
</body>
</html>'''
