"""Generate all plots for td3_mfris_jam_v3 (1000 episodes, 4-module RIS optimization)"""
import matplotlib
matplotlib.use('Agg')
...
path = 'data/storage/scratch/td3_mfris_jam_v3'
...
ax.set_title('UAV Trajectory - 4-Module RIS Optimization (v3, 1000 eps)', color='#60a5fa', fontsize=14, pad=15)
...
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, 'trajectory.png'), dpi=150, facecolor='#0f172a')
plt.close(); print("Saved trajectory.png")
...
if os.path.exists(f_last):
    ...
    for elem in range(4):
        ...
        ax.set_title(f'RIS Element #{elem} Phase & Magnitude (v3)', color='#93c5fd', fontsize=12, pad=10)
