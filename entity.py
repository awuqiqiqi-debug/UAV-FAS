...
class RIS(object):
    ...
    def calculate_jamming_power(self, H_br=None, h_rp=None, G=None, h_FR=None, F=None):
        ...
        RIS干扰原理：theta_J放大热噪声(P_J)产生人工噪声，独立于数据信号。
        干扰功率 = ||h_R_p^H * theta_J||^2 * P_J

        返回:
            jamming: 干扰噪声功率 (mW)
        """
        P_J_mw = self._dbm_to_mw(self.P_J)
        if h_rp is not None:
            h_rp_H = h_rp.H  # (1, RIS_ant_num)
            # 干扰功率 = 信道增益 * RIS放大增益 * 噪声功率
            jamming = math.pow(np.linalg.norm(h_rp_H * self.theta_J), 2) * P_J_mw
        else:
            jamming = math.pow(np.linalg.norm(self.theta_J, 'fro'), 2) * P_J_mw
        return jamming
