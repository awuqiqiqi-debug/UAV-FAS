...
"""
系统模型图 v6
1. 上方DRL框/标题/参数文字不遮挡，垂直间距拉开
2. FAS文字放在无人机下方，不挡住激活端口
3. 窃听者往屏幕前方偏移(3D错觉)
4. 图例移到左上角
"""
import matplotlib
...
ax.imshow(gradient, aspect='auto', cmap=plt.cm.Blues,
          extent=[-1.0, 16.0, 11.5, 13.5], alpha=0.15, zorder=0)
...
#  窃听者 (地面, 往屏幕前方偏移, 3D错觉)
# ═══════════════════════════════════════════
eve_x, eve_y = 9.5, 0.3  # 往下偏移=往屏幕前方

# 阴影 (更大, 模拟近处)
eve_shadow = FancyBboxPatch((eve_x-0.16+0.05, eve_y-0.26+0.05), 0.32, 0.52,
                            boxstyle="round,pad=0.02",
                            facecolor='black', alpha=0.15, zorder=3)
ax.add_patch(eve_shadow)
# 手机 (稍大, 模拟近处)
eve_phone = FancyBboxPatch((eve_x-0.16, eve_y-0.26), 0.32, 0.52,
                           boxstyle="round,pad=0.02",
                           facecolor='#E53935', edgecolor='white', linewidth=1.2, zorder=5)
ax.add_patch(eve_phone)
eve_screen = Rectangle((eve_x-0.12, eve_y-0.18), 0.24, 0.36,
                       facecolor='white', edgecolor='#E53935', linewidth=0.4, alpha=0.9, zorder=6)
ax.add_patch(eve_screen)
# 窃听图标
ax.plot([eve_x-0.05, eve_x+0.05], [eve_y+0.05, eve_y+0.05], color='#E53935', lw=1.5, zorder=7)
ax.plot([eve_x-0.025, eve_x+0.025], [eve_y+0.05, eve_y+0.05], color='#E53935', lw=3, zorder=7)
ax.text(eve_x, eve_y-0.4, 'Eavesdropper $p$', fontsize=8, ha='center', va='top',
...
# FAS → Eve
ax.annotate('', xy=(eve_x, eve_y+0.3), xytext=(uav_x+0.5, fas_y-0.3),
            arrowprops=dict(**jam_kw))
ax.text(9.8, 6.5, '$\\mathbf{h}^{U}_{U,p}$', fontsize=9, color='#E53935',
        ha='center', style='italic',
        bbox=dict(fc='white', ec='none', alpha=0.85, pad=1.5, boxstyle='round,pad=0.2'))
...
ax.annotate('', xy=(eve_x-0.1, eve_y+0.3), xytext=(ris_x+0.5, ris_y-ris_h/2),
            arrowprops=dict(**jam_kw))
ax.text(9.0, 3.3, '$\\mathbf{h}^{U}_{R,p}$', fontsize=9, color='#E53935',
        ha='center', style='italic',
        bbox=dict(fc='white', ec='none', alpha=0.85, pad=1.5, boxstyle='round,pad=0.2'))
...
# ═══════════════════════════════════════════
# DRL框 (最上方)
drl_x, drl_y = 7.5, 13.0
drl_w, drl_h = 4.5, 0.55
...
ax.text(drl_x, drl_y+0.1, 'Dual-Agent Twin-TD3', fontsize=9,
        ha='center', va='center', fontweight='bold', zorder=6)

a1_box = FancyBboxPatch((drl_x-2.0, drl_y-0.25), 1.4, 0.28,
                        boxstyle="round,pad=0.04",
                        facecolor='#E3F2FD', edgecolor='#2196F3', linewidth=0.8, zorder=6)
ax.add_patch(a1_box)
ax.text(drl_x-1.3, drl_y-0.11, 'Agent 1 (FAS+RIS)', fontsize=6.5,
        ha='center', va='center', color='#1565C0', zorder=7)

a2_box = FancyBboxPatch((drl_x+0.6, drl_y-0.25), 1.4, 0.28,
                        boxstyle="round,pad=0.04",
                        facecolor='#E8F5E9', edgecolor='#4CAF50', linewidth=0.8, zorder=6)
ax.add_patch(a2_box)
ax.text(drl_x+1.3, drl_y-0.11, 'Agent 2 (UAV)', fontsize=6.5,
        ha='center', va='center', color='#2E7D32', zorder=7)

# 标题 (DRL框下方, 有间距)
ax.text(7.5, 12.2, 'FAS辅助主动RIS无人机安全通信系统模型',
        fontsize=14, ha='center', va='center', fontweight='bold', color='#1a1a1a')

# 参数 (标题下方, 有间距)
params = '$f_c$=28GHz | FAS: $N$=12 | RIS: $M$=64 | $K$=2 | $P$=1 | $H$=50m'
ax.text(7.5, 11.6, params, fontsize=7.5, ha='center', va='center', color='#666',
        bbox=dict(fc='#f5f5f5', ec='#ccc', alpha=0.9, boxstyle='round,pad=0.3', lw=0.8))

# ═══════════════════════════════════════════
#  图例 (左上角)
# ═══════════════════════════════════════════
legend_x, legend_y = -0.3, 13.0
legend_items = [
    ('#2196F3', '-', '信号链路 (LOS)'),
    ('#E53935', '--', '窃听/干扰链路'),
    ('#4CAF50', 'o', 'FAS 激活端口'),
...
leg_bg = FancyBboxPatch((legend_x-0.2, legend_y-len(legend_items)*0.35-0.2), 2.6, len(legend_items)*0.35+0.45,
                        boxstyle="round,pad=0.08",
                        facecolor='white', edgecolor='#BDBDBD', linewidth=0.8, alpha=0.95, zorder=5)
ax.add_patch(leg_bg)
ax.text(legend_x+1.1, legend_y+0.05, '图例', fontsize=8, ha='center', va='center',
...
zone2 = FancyBboxPatch((8.2, -0.8), 3.0, 2.0, boxstyle="round,pad=0.1",
                       facecolor='none', edgecolor='#E53935', linewidth=0.8,
                       linestyle='--', alpha=0.4, zorder=2)
ax.add_patch(zone2)
ax.text(9.7, 1.4, '干扰抑制区', fontsize=7, ha='center', color='#E53935', alpha=0.6, zorder=3)

# ═══════════════════════════════════════════
#  保存
# ═══════════════════════════════════════════
output_dir = r"C:\Users\红\Desktop\0606强化学习用于保密能源——流体天线辅助无人机保密通信"
fig.savefig(f"{output_dir}/系统模型_v6.pdf", bbox_inches='tight', pad_inches=0.3, dpi=300)
fig.savefig(f"{output_dir}/系统模型_v6.png", bbox_inches='tight', pad_inches=0.3, dpi=300)
print(f"PDF: {output_dir}/系统模型_v6.pdf")
print(f"PNG: {output_dir}/系统模型_v6.png")
plt.close()
